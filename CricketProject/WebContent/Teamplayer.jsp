...
<% session.setAttribute("add","search"); %>
<form action="Teamplayer.jsp" method="post">
<%--get Id,name,number --%>
<% String id="";
   String mobilenumber="";
   String firstname="";
...
<%Connection con=DBconnection.getDBConnection(); %>
 ...
 displayteam=(String) session.getAttribute("teamname");
            if (con != null) {
				try {
	...
	<nav id="nav">
			<div class="innertube">
				...
				<ul>
					...
					<li><a href="TeamPlayer.jsp">view Team Players</a></li>	
...
<tr>
        <th><b><center><font color="blue" size="4">Player Id</font></center></b></th>
		<th><b><center><font color="blue" size="4">PlayerName</font></center></b></th>
		...
		<% String name="";
            ...
            if (con != null) {
				try {
					Statement st = con.createStatement();
					 ResultSet rs = st.executeQuery("select name,role,id from player where teamname='"+displayteam+"'");
					 while (rs.next()) 
					{
					name=rs.getString(1);
					role=rs.getString(2);
					pid=rs.getString(3);
					  ...
					  { %>
						  <tr>
     						 <td><center><input type="text" value="<%=pid%>" name=<%="id"+i %> style="border:0;font-weight: bold; border-color:transparent; text-align:center" size="10" readonly="value" ></center></td>
                               
                              <td><center> <input type="text" value="<%=name%>" name=<%="name"+i %> style="border:0;font-weight: bold; border-color:transparent;text-align:center" size="10"  readonly="value" ></center></td>
